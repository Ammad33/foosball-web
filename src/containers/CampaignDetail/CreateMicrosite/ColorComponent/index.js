...
const ColorComponent = ({ heading, value, handlValue, onClick, open, onChangeComplete, bottom }) => {
    ...
    return (
        ...
        <div className={styles.colorContainer} style={stle1} >
            ...
            {open && bottom === true && <div><div style={style}><ChromePicker color={value} onChangeComplete={onChangeComplete} /> < X onClick={onClick} style={{ float: 'right' }} /></div></div>}
            <h6>{heading}</h6>
            ...
            {open && bottom === undefined && <div>< X onClick={onClick} />  <div style={style}> <ChromePicker color={value} onChangeComplete={onChangeComplete} /></div></div>}
        </div >
